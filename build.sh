...
[ $MODEL = "S9" ] && cat $WORKDIR/arch/arm64/configs/starlte_defconfig >> $KERNELCONFIG
...
if [ $MODEL = "S9" ]; then
	sudo sed -i -- 's/SRPQH16A002KU/SRPQH16B002KU/g' $WORKDIR/ramdisk/split_img/boot.img-board
	cd $WORKDIR/ramdisk
	./repackimg.sh
else
	cd $WORKDIR/ramdisk
